...
plt.title("ETR Predicted vs Actual")
plt.xlabel("Actual Values (Pa)")
plt.ylabel("Predicted Values (Pa)")
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(save_dir, f'ETR_Predicted_vs_Actual_{current_time}.png'))
plt.show()
...
plt.title("ETR Prediction Error Distribution")
...
cv_scores = cross_val_score(etr_model, X_train, y_train, scoring='neg_mean_squared_error', cv=kf)
...
train_sizes, train_scores, test_scores = learning_curve(
    etr_model, X_train, y_train, cv=kf, scoring='neg_mean_squared_error', 
...
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(save_dir, f'Learning_Curve_ETR_{current_time}.png'))
plt.show()
...
predicted_pressure = etr_model.predict(custom_data)
